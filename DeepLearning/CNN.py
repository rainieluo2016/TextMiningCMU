...
def read_input(path, word_dict=None, preprocess=True):
    ...
    if word_dict is None:
        ...
        word_count = [('UNK', 0)] + sorted([(key, value) for key, value in word_count.items()], key=lambda x: (-x[1], x[0]))[:9999]
...
class Net(nn.Module):
    ...
    def __init__(self, embedding):
        ...
        self.conv1 = nn.Conv2d(1, 100, kernel_size=(3, 100), padding=(1, 0))
        self.relu = nn.ReLU()
        self.fc1 = nn.Linear(in_features=100, out_features=2)
...
if __name__ == "__main__":
    ...
    for epoch in range(n_epochs):
        ...
        print('Epoch {0}: train_accu {1}, test_accu {2}, accumulative time: {3}s'.format((epoch + 1), train_accu,
                                                                                         test_accu,
    ...
    embedding_dict['</s>'] = np.random.normal(size=embedding_dict['</s>'].shape)
